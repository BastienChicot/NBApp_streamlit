...
def stat_20matchs_splits(full_name):
    ...
    y = code_base[2:3]
    code = ''.join(y)
    last20=data_saison.loc[data_saison['joueur']==str(code)]

    last20=last20.sort_values(by=['Date'])
    indexNames = last20[last20['Rk'] == 'Rk' ].index
    # Delete these row indexes from dataFrame
    last20.drop(indexNames , inplace=True)
    last20.info()
    last20=last20[["Date","Tm","Opp","GS","MP","PTS","FGA","AST","TRB","BLK","STL","TOV","PF"]]

    last20=last20.tail(20)
    return(last20)
    # annee = '2021'
...
def stat_teams (player_team,Opp):
    ...
    L = pd.read_csv("data/ranking_team.csv",sep=";")
    team_code=pd.read_csv('data/code_team.csv', sep=';')
    
    L = pd.merge(L,team_code, on='TEAM')
